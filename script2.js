...
fetch(apiLink2).then((response) => response.json().then(cada => {

    listaPaises = Object.entries(cada.dimension.geo.category.index).map(([siglas, id]) => ({ siglas, id }));

    consumicion = Object.values(cada.value).slice(1069, 1109);
    delete consumicion[29]; //omitir a Liechtenstein

    listaPaises.map((pais, index) => {
        pais.nombre = Object.values(cada.dimension.geo.category.label)[index];
    })
